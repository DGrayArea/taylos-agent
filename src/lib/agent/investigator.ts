...
export function investigateAnomaly(
  ...
  const confidence = calculateConfidence(
    anomaly,
    evidence,
    diagnosis.hypothesis_confidence,
  );
  ...
  return {
    ...
    step_3_classification: generateClassification(
      diagnosis.primary_category,
      anomaly,
    ),
...
function generateFactualSummary(
  anomaly: Anomaly,
  model: MasterUnifiedModel,
): string {
  const txns = anomaly.affected_transactions
    .map((id) => model.unified_data.transactions.find((t) => t.id === id))
    .filter(Boolean) as UnifiedTransaction[];

  if (anomaly.type.includes("Duplicate")) {
    const t1 = txns[0];
    const t2 = txns[1] || t1;
    const rowInfo = t1?.source_line ? ` row ${t1.source_line}` : "";
    const secondRowInfo = t2?.source_line ? ` row ${t2.source_line}` : "";
    return `Two ${formatNaira(t1.amount)} charges appear for ${t1.merchant} dated ${t1.date.split("T")[0]} in ${t1.source_document}${rowInfo} and ${t2.source_document}${secondRowInfo}. Only one payment was expected.`;
  }

  if (anomaly.type.includes("Mismatch")) {
    return `A transaction for ${formatNaira(anomaly.affected_amounts[0])} at ${txns[0]?.merchant || "vendor"} does not match the invoice amount of ${formatNaira(anomaly.affected_amounts[1])}. This is a ${formatNaira(Math.abs(anomaly.affected_amounts[0] - anomaly.affected_amounts[1]))} difference.`;
  }
...
function diagnoseRootCause(anomaly: Anomaly, model: MasterUnifiedModel) {
  ...
  if (anomaly.type.includes("Duplicate")) {
    ...
    if (txns.length >= 2) {
      const timeDiff =
        Math.abs(
          new Date(txns[1].date).getTime() - new Date(txns[0].date).getTime(),
        ) /
        (1000 * 60 * 60);
      if (timeDiff < 2) {
        diagnosis =
          "Payment processor system retry: A timeout or connection issue on the first attempt triggered an automatic retry. Both attempts eventually succeeded, resulting in a duplicate charge.";
        primaryHypothesis = "System Retry Error";
        confidence = 95;
        category = "BILLING_ERROR";
      } else {
        diagnosis =
          "Possible customer error or intentional multiple purchase. The time gap between charges suggests two separate sessions or submissions.";
    ...
    diagnosis =
      "Missing documentation: A charge was processed through the banking system without a corresponding invoice or receipt being captured in the intake records.";
    primaryHypothesis = "Missing Record";
    confidence = 80;
    category = "DATA_ANOMALY";
  } else if (
    anomaly.type.includes("High-Risk") ||
    anomaly.type.includes("Geographic")
  ) {
    diagnosis =
      "Potential unauthorized activity: Transaction characteristics (merchant type or location) deviate significantly from the established account baseline.";
  ...
  return {
    ...
    alternative_hypotheses: [
      {
        hypothesis: "Manual Data Entry Error",
        likelihood: "low",
        evidence_against: "System logs show automated processing",
      },
      {
        hypothesis: "Legitimate one-off transaction",
        likelihood: "moderate",
        evidence_against: "No prior history with this merchant category",
      },
    ],
  };
}
...
function buildEvidenceChain(
  ...
  if (txns.length >= 2) {
    ...
    temporal.push({
      ...
      significance:
        timeDiff < 120
          ? "Matches standard processor retry pattern"
          : "Suggests separate sessions",
  ...
  const matchingComplaint = model.unified_data.complaints.find(
    (c) =>
  ...
  return {
    total_evidence_points:
      temporal.length + merchant.length + system.length + customer.length,
    supporting_evidence: [
...
function assessImpact(anomaly: Anomaly, category: string) {
  ...
  return {
    financial_impact: `${formatNaira(amount)} overcharged to the customer account.`,
    customer_impact:
      "Account overcharged, increasing the risk of overdraft or cash flow issues.",
    operational_impact:
      "Manual reconciliation and refund processing may be required.",
    regulatory_impact:
      "Dispute resolution should follow local financial regulations.",
    overall_severity: anomaly.severity,
    impact_score:
      anomaly.severity === "CRITICAL" ? 9 : anomaly.severity === "HIGH" ? 7 : 4,
  };
}
