...
export function normalizeMerchant(merchantStr: string): string {
  ...
  if (cleaned.includes("STARBUCKS") || cleaned.includes("SBUX")) return "Starbucks";
  if (cleaned.includes("APPLE") || cleaned.includes("AAPL")) return "Apple";
  
  // Generic cleanup: Remove .COM, INC, LLC
  cleaned = cleaned.replace(/\.COM/g, "").replace(/ INC\.?/g, "").replace(/ LLC\.?/g, "");
 ...
 */
export function normalizeAmountAndDirection(amountRaw: any, typeRaw?: string): { amount: number, direction: 'debit' | 'credit' } {
  let amount = 0;
  ...
  if (typeRaw) {
    ...
    if (t === 'CREDIT' || t === 'CR' || t === 'INFLOW' || t === 'DEPOSIT') direction = 'credit';
    if (t === 'DEBIT' || t === 'DR' || t === 'OUTFLOW' || t === 'PAYMENT' || t === 'WITHDRAWAL') direction = 'debit';
  }
