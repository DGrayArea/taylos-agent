...
export function analyzeFinancialData(
  ...
  const detectedAnomalies =
    unifiedModel.unified_data.anomalies?.anomalies_detected || [];
  ...
  return {
    analysis_metadata: {
      ...
      total_transactions_analyzed:
        unifiedModel.unified_data.transactions.length,
    },
    source_documents: payloads.map((payload) => payload.filename),
    feature_1_data_intake: unifiedModel,
    feature_2_anomalies: {
      total_anomalies_found: detectedAnomalies.length,
      anomalies_by_severity: {
        critical: detectedAnomalies.filter((a) => a.severity === "CRITICAL")
          .length,
...
function generateRecommendations(investigations: Investigation[]) {
  const primaryRec = investigations.some(
    (i) => i.step_3_classification.requires_refund,
  )
  ...
  if (
    investigations.some(
      (i) => i.step_3_classification.primary_category === "BILLING_ERROR",
    )
  ) {
    secondaryActions.push(
      "Escalate to payment processor for retry bug investigation.",
    );
    secondaryActions.push(
      "Check for other customers affected by same processor timeout.",
    );
  }
  if (
    investigations.some(
      (i) => i.step_3_classification.primary_category === "FRAUD",
    )
  ) {
    secondaryActions.push(
      "Freeze affected account and initiate secondary verification.",
    );
  }
  if (
    investigations.some(
      (i) => i.step_3_classification.primary_category === "DATA_ANOMALY",
    )
  ) {
    secondaryActions.push(
      "Improve document capture quality or request original data sources.",
    );
  }
...
function generateExecutiveSummary(
  ...
  return {
    ...
    root_cause: mainInvestigation
      ? mainInvestigation.step_2_root_cause.primary_hypothesis
      : "Normal business activity",
    confidence: mainInvestigation
      ? `${mainInvestigation.step_6_confidence.overall_confidence_percentage}%`
      : "100%",
    recommended_action: mainInvestigation
      ? "Approve recommended resolution actions"
      : "No action needed",
    priority: highestSeverity,
    next_steps: mainInvestigation
      ? "Execute refunds and resolve system conflicts"
...
function getHighestSeverity(investigations: Investigation[]): SeverityLevel {
  if (
    investigations.some(
      (i) => i.step_5_impact_assessment.overall_severity === "CRITICAL",
    )
  )
    return "CRITICAL";
  if (
    investigations.some(
      (i) => i.step_5_impact_assessment.overall_severity === "HIGH",
    )
  )
    return "HIGH";
  if (
    investigations.some(
      (i) => i.step_5_impact_assessment.overall_severity === "MEDIUM",
    )
  )
    return "MEDIUM";
  if (
    investigations.some(
      (i) => i.step_5_impact_assessment.overall_severity === "LOW",
    )
  )
    return "LOW";
