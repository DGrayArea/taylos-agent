...
import { LayoutDashboard, UploadCloud, History, Settings, HelpCircle, Beaker } from "lucide-react";
...
const navItems = [
  ...
  { icon: HelpCircle, label: "Help/Doc", href: "/help" },
];
  ...
  return (
    <>
      ...
      <aside className={cn(
        ...
        <Beaker className="w-6 h-6 text-[var(--color-gold)] mr-3 flex-shrink-0" />
        <span className="font-bold text-lg tracking-wide truncate">Taylos agent</span>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <item.icon className={cn("w-5 h-5 mr-4 relative z-10 transition-colors", isActive ? "text-[var(--color-gold)]" : "group-hover:text-gray-300")} />
      ...
      <div className="p-6 border-t border-white/5">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[var(--color-gold)] to-[var(--color-gold-light)] flex items-center justify-center text-[var(--color-navy)] font-bold">
            JD
          </div>
          <div className="flex flex-col">
            <span className="text-sm font-medium">Jane Doe</span>
            <span className="text-xs text-gray-400">Financial Analyst</span>
          </div>
        </div>
      </div>
    </aside>
